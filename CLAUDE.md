...
- **Deployment status: LIVE.** Cindy approved applying this to production same-session. Migration applied; `classify-heart-note-theme` redeployed (v5, `verify_jwt: false` preserved to match its trigger-only usage) and `generate-daily-grace-notes` redeployed (v13). Verified end-to-end: both `heart_notes_classify_theme_on_insert`/`_on_summary` triggers confirmed present and enabled; 0 profiles left with the stale `[]` default. **Backfilled Cindy's 4 existing Heart Notes** by invoking `classify-heart-note-theme` directly for each — her `profiles.inferred_themes` now holds real tags (gratitude, growth, expansion, presence at weight 1; hope, faith, contentment at weight 0.5), confirming the full read/write path works. Did **not** force-backfill her daily chat (her most recent chat day was 2026-08-09, two days back) because the only way to trigger `classifyDailyChatTheme` is through `generate-daily-grace-notes` itself, which upserts a real `daily_grace_notes` row for every onboarded user for whatever date is passed — running it against a past date risked silently overwriting notes 10 users already received. Daily chat classification will pick up naturally at the next 1am UTC cron run (or any night she chats going forward) — no action needed.
...
- **New route `src/routes/sitemap[.]xml.ts`** (escaped-dot filename → literal `/sitemap.xml` path): emits the 25 hand-curated static entries (carried over verbatim from the old file, `/library/devotional` bumped to `changefreq=daily`) plus one `<url>` per row in `daily_devotionals` at `/library/devotional/YYYY-MM-DD` (lastmod = the devotional's date, future-dated rows excluded). Dates are read over PostgREST with the hardcoded anon key (the table is public-select) — no server secret dependency; on any fetch failure the route still serves the static entries. `Cache-Control: public, max-age=3600`.
- **Deleted `public/sitemap.xml`** — the static asset would have shadowed the route.
- robots.txt unchanged (already points at `/sitemap.xml`). routeTree regenerated by the build.
- **Verified via `wrangler dev` against the production build output:** 200, `application/xml`, 42 URLs = 25 static + 17 live devotional dates (2026-07-22 newest).
- ⚠️ Goes live on next Lovable publish. After that, resubmit the sitemap in Google Search Console once; from then on new devotionals need no action.
